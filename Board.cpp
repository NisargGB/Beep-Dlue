...
tuple <int, int, int, int, int, int, int, int, int, int, int> Board::cannonsAndSoldierAttacks(bool side)
{
	...
	set<pair<int,int>> hash_Set1;
	set<pair<int,int>> hash_Set2;
	set<pair<int,int>> hash_Set3;
	if(!side)
		reverseBoard();
	...
	return make_tuple(vertCannons, horizCannons, diagCannons, cannonAttack, soldierAttackNormal, soldierAttackRetreat, cannonAttackingPosition, hash_Set1.size(), hash_Set2.size(), hash_Set3.size(),0);
...
int Board::heuristicScore()
{
	...
	int numOfSoldierOnAttack, numOfEnemiesOnAttack, numOfSoldierUnderAttack, numOfEnemiesUnderAttack;
	...
	//to calculate the number of soldiers on attack and under attack and goals on attack and townhalls under attack
	...
	// a4 return 4*vertCannons + 3*diagCannons + 1*horizCannons + 3*cannonAttack + 2*cannonAttackingPosition + 6*soldierAttackRetreat + 2*soldierAttackNormal + 100*(soldierCount - enemyCount) + 50*soldierCount + 2000*(townhallCount - goalCount);
	...
	return 4*vertCannons + 3*diagCannons + 1*horizCannons + 3*cannonAttack + 2*cannonAttackingPosition + 6*soldierAttackRetreat + 10*(numOfSoldierOnAttack - numOfEnemiesOnAttack) + 10*(numOfEnemiesUnderAttack - numOfSoldierUnderAttack) + 100*(soldierCount - enemyCount) + 50*soldierCount + 2000*(townhallCount - goalCount);
